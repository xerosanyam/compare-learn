    ...
    <div class="max-w-5xl px-4 pt-4 pb-64 mx-auto prose-sm prose">
      ...
      <div v-for="page in pages" :key="page" class="pb-8 border-b">
        <div class="flex flex-row space-x-8 sm:space-x-32">
          <ContentDoc v-if="data1[page]" :id="page" class="flex-1 overflow-hidden" :path="data1[page]._path" />
          <div v-else :id="page" class="flex items-center justify-center flex-1 h-40">
            <div>&nbsp;</div>
          </div>
          <ContentDoc v-if="data2[page]" class="flex-1 overflow-hidden" :path="data2[page]._path" />
...
<script setup>
const url1 = 'computer-science/paradigms/object'
const url2 = 'computer-science/paradigms/functional'
