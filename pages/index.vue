<template>
  <div class="px-4 pt-4 pb-64 mx-auto prose-sm prose sm:prose lg:prose-lg">
    <h1 class="text-lg">Compare & Learn</h1>
    <p>
      The idea behind compare & learn is to build on top of exisiting knowledge.
      <br />
      If you know React, it makes it comparatively easier to learn Vue.
      <br />
      If you know Java, it makes it comparatively easier to learn Javascript.
    </p>
    <div>
      <nuxt-link to="/programming-languages">Programming Languages [Java vs Javascript]</nuxt-link>
    </div>
    <div>
      <nuxt-link to="/computer-science/paradigms">Computer Science -> Paradigms [Object vs Functional]</nuxt-link>
    </div>
    <div>
      <nuxt-link to="/react-vs-svelte">UI Libraries [React vs Svelte]</nuxt-link>
    </div>
    <div>
      <nuxt-link to="/react-vs-vue">UI Libraries [React vs Vue]</nuxt-link>
    </div>
    <div>
      <nuxt-link to="/concurrency">Concurrency (how JS works)</nuxt-link>
    </div>
    <div>
      <nuxt-link to="/mixed">Mixed</nuxt-link>
    </div>
    <div>
      <nuxt-link to="/ds">Data Structures & Algorithms Notes (Updated: 11 May 2024)</nuxt-link>
    </div>
    <div>
      <nuxt-link to="/leetcode">Leetcode Answers (Updated: 11 May 2024)</nuxt-link>
    </div>
  </div>
</template>
